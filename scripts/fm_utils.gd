...
static func _pack_files(mission:Mission, scene_tree:SceneTree) -> logs.ErrorReport:
	var zipper := ZIPPacker.new()
	var pakpath := Path.join(mission.path_root, mission.zipname)

	var err := zipper.open(pakpath)
	if err != OK:
		return logs.ErrorReport.new(false, "Couldn't create pk4 archive at '%s'" % [pakpath])

	for fpath:String in mission.filepaths:
		var rel_path := fpath.trim_prefix(mission.path_root + '/')
	...
	zipper.close()
	return logs.ErrorReport.new(true)
...
static func build_file_tree(mission:Mission) -> void:
	...
	var ign_dirs := fm_manager.default_ignored_directories.duplicate()
	var ign_files := fm_manager.default_ignored_files.duplicate()

	init_ignores(mission.pkignore, ign_dirs, ign_files)

	_add_files_to_ignores(mission.path_root, ign_dirs, ign_files)
	_add_maps_to_ignores(mission, ign_dirs, ign_files)

	var root := FMTreeNode.new(mission.path_root)
...
static func _build_mission_nodes(mission:Mission, parent:FMTreeNode, ign_dirs:Set, ign_files:Set) -> void:
	...
	for dir:String in dpaths:
		...
		dir_node.ignored = parent.ignored \
						 ...
						 or parent.path.ends_with(Path.join(mission.path_root, "maps"))
...
static func _add_maps_to_ignores(mission:Mission, ign_dirs:Set, ign_files:Set) -> void:
	if not Path.dir_exists(mission.path_maps): return
	var dpaths:Array[String] = Path.get_dirpaths(mission.path_maps)
	var fpaths:Array[String] = Path.get_filepaths(mission.path_maps)

	# every dir inside '/maps' should be ignored
	for dir:String in dpaths:
		ign_dirs.add( Path.join(mission.path_maps, dir) )

	var used_map_names := mission.map_names
