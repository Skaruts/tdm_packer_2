...
var colors := {
	...
	caller   = "[color=%s]" % [Color(0.71764707565308, 0.4705882370472, 1).to_html(false)],    # b778ff
	success  = "[color=%s]" % [Color(0.47843137383461, 1, 0.4705882370472).to_html(false)],    # 7aff78
	task     = "[color=%s]" % [COLOR_TASK.to_html(false)],    # ffbb78
}


class ErrorReport extends RefCounted:
	var ok:bool
	var error:String
...
# provided in 'messages', and report the benchmark time
func run(f:Callable, messages:={}, silent:=false) -> Variant:
	if silent: return f.call()
	var has_messages := not messages.is_empty()
	...
	if has_messages:
		if not res is ErrorReport or res.ok:
